...
Добавь примеры кода для каждого проекта и объясни каждый шаг реализации.`
  ...
  // Get the block based on index
  let block;
  if (blockIndex === 11) {
    block = block11;
  } else {
    // ... keep existing code for other blocks
    const blocks = [
      {
  ...
  if (!lesson.prompt) {
    console.error(`No prompt found for lesson: ${lessonId}`);
    throw new Error(`Промпт для урока ${lessonId} не найден. Пожалуйста, свяжитесь с поддержкой.`);
  }
